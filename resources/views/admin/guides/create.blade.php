...
@section('title', isset($guide) ? 'تعديل الدليل' : 'دليل جديد')
@section('subtitle', 'صمم خطوات مساعدة احترافية لعملائك')
...
<div class="max-w-6xl mx-auto" x-data="{ 
    steps: {{ isset($guide) ? $guide->steps->map(fn($s) => ['id' => $s->id, 'image_path' => $s->image_path, 'description' => $s->description])->toJson() : '[]' }} 
}">
    <form action="{{ isset($guide) ? route('admin.guides.update', $guide->id) : route('admin.guides.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        @csrf
        ...
        <div class="lg:col-span-8 space-y-8">
            <div class="bento-card p-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="icon" value="{{ old('icon', $guide->icon ?? 'ph-monitor') }}" class="bento-input" dir="ltr" required>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div x-show="steps.length === 0" class="text-center py-20 bg-slate-50 dark:bg-white/[0.01] border-2 border-dashed border-slate-100 dark:border-white/5 rounded-[2rem]">
                        <i class="ph ph-image-square text-5xl text-slate-200 mb-4 block mx-auto"></i>
                        <p class="text-slate-400 font-bold text-sm">لم يتم إضافة أي خطوات</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-4 space-y-8">
            <div class="bento-card p-8">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $guide->sort_order ?? 0) }}" class="bento-input">
                    </div>
                    ...
                    <div class="p-5 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_active" {{ old('is_active', $guide->is_active ?? true) ? 'checked' : '' }} class="sr-only peer">
            ...
            <button type="submit" class="bento-button !w-full !h-14">
                ...
                {{ isset($guide) ? 'تحديث الدليل' : 'حفظ ونشر الدليل' }}
            </button>
        </div>
    </form>
</div>
