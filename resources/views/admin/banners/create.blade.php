...
@section('title', isset($banner) ? 'تعديل البنر' : 'بنر جديد')
...
<div class="max-w-6xl mx-auto">
    <form action="{{ isset($banner) ? route('admin.banners.update', $banner->id) : route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        @csrf
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            <div class="bento-card p-10">
                ...
                <div class="space-y-6">
                    <div id="imagePreviewContainer" class="{{ isset($banner) && $banner->full_image_url ? '' : 'hidden' }} relative w-full h-64 rounded-2xl overflow-hidden border border-slate-100 dark:border-white/5 mb-4 group shadow-xl">
                        <img id="imagePreview" src="{{ isset($banner) && $banner->full_image_url ? $banner->full_image_url : '#' }}" class="w-full h-full object-cover">
                    ...
                    <div class="p-10 border-2 border-dashed border-slate-100 dark:border-white/5 rounded-3xl text-center group hover:border-brand-500 transition-all bg-slate-50 dark:bg-white/[0.01] cursor-pointer" onclick="document.getElementById('bannerImageInput').click()">
                        ...
                        <p class="text-sm font-black text-slate-900 dark:text-white">رفع صورة البنر</p>
        ...
        <div class="lg:col-span-4 space-y-8">
            <div class="bento-card p-8">
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $banner->sort_order ?? 0) }}" class="bento-input">
                    </div>

                    <div class="space-y-2">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest px-1">أيقونة العرض (اختياري)</label>
                        <input type="text" name="icon" value="{{ old('icon', $banner->icon ?? '') }}" placeholder="ph-presentation-chart-fill" class="bento-input" dir="ltr">
                        <p class="text-xs text-slate-500 mt-2">اكتب اسم أيقونة Phosphor (مثلاً: ph-presentation-chart-fill) أو اتركه افتراضياً.</p>
                    </div>
                    ...
                    <div class="p-5 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_active" {{ old('is_active', $banner->is_active ?? true) ? 'checked' : '' }} class="sr-only peer">
            ...
            <button type="submit" class="bento-button !w-full !h-14">
                ...
                {{ isset($banner) ? 'تحديث البنر' : 'حفظ البنر' }}
            </button>
        </div>
    </form>
</div>
