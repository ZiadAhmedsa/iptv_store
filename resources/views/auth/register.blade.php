                ...
                @if(session('show_register_verification'))
                    <div class="space-y-8">
                        <div class="luxury-glass p-8 rounded-[2.5rem] border-brand-500/20 text-center space-y-6 relative overflow-hidden">
                            ...
                            <form action="{{ route('register.reset') }}" method="POST" id="reset-register-form" class="hidden">@csrf</form>
                            <button type="button" onclick="document.getElementById('reset-register-form').submit()" class="text-[10px] font-black text-brand-500 hover:text-brand-600 transition-colors uppercase tracking-widest flex items-center justify-center gap-2 mx-auto">
                ...
                @else
                    <form method="POST" action="{{ route('register') }}" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Name -->
                            <div class="space-y-3">
                                <label for="name" class="text-xs font-black uppercase text-slate-400 tracking-widest px-2">الاسم الكامل</label>
                                <div class="relative group">
                                    ...
                                    <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus placeholder="الاسم الكريم"
                            ...
                            <!-- Phone -->
                            <div class="space-y-3">
                                <label for="phone" class="text-xs font-black uppercase text-slate-400 tracking-widest px-2">رقم الهاتف (الواتساب)</label>
                                <div class="relative group">
                                    <div class="absolute inset-y-0 right-0 pr-6 flex items-center pointer-events-none text-slate-400 group-focus-within:text-brand-500 transition-colors">
                                        <i class="ph-bold ph-phone text-xl"></i>
                                    </div>
                                    <input id="phone" type="text" name="phone" value="{{ old('phone') }}" required placeholder="+966..."
                                        class="w-full bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-2xl pl-6 pr-14 py-4.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500 transition-all font-bold">
                                </div>
                                @error('phone')
                                    <span class="text-red-500 text-[10px] font-black block px-2">{{ $message }}</span>
                            ...
                            <!-- Confirm Password -->
                            <div class="space-y-3">
                                <label for="password_confirmation" class="text-xs font-black uppercase text-slate-400 tracking-widest px-2">تأكيد كلمة المرور</label>
                                <div class="relative group">
                                    <div class="absolute inset-y-0 right-0 pr-6 flex items-center pointer-events-none text-slate-400 group-focus-within:text-brand-500 transition-colors">
                        ...
                        <div class="text-center pt-2">
                            <p class="text-xs font-black text-slate-400">
                                لديك حساب بالفعل؟ 
                                <a href="{{ route('login') }}" class="text-brand-500 hover:text-brand-600 transition-colors font-bold">سجل دخولك الآن</a>
                            </p>
                        </div>
                    </form>
                @endif
